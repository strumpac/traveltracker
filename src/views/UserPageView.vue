<script setup>
    ...
    let allTickets = [];
    let oldTickets = [];
    let currentTickets = []; //ora int array poi da fare con dati component
    let username = "";
    let userPfpSrc = "";
    ...
    function getUserData(){
        ...
        allTickets = [1753408969304,1745408969303,2743408969303,1743408969103,743408969303]; //COSA SONO STI NUMERI
    ...
    function fillTicketsLists(){
        for(let i = 0; i< allTickets.length; i++){
            if(allTickets[i] > timeNow){
                currentTickets.push(new Date(allTickets[i]).getMinutes() + ":" +new Date(allTickets[i]).getUTCHours() + " " +new Date(allTickets[i]).getDate() + "/" + new Date(allTickets[i]).getMonth() + "/" + new Date(allTickets[i]).getFullYear());
            }else{
                oldTickets.push(new Date(allTickets[i]).getDate() + "/" + new Date(allTickets[i]).getMonth() + "/" + new Date(allTickets[i]).getFullYear());
            }
        }
    }
...
<template>
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <!--Riga 2-->
            <div class="col-6">
                <Ticket v-for="ct in currentTickets" :dateTime="ct"></Ticket>
            </div>
            <div class="col-6">
                <Ticket v-for="ct in oldTickets" :dateTime="ct"></Ticket>
            </div>
        </div>
